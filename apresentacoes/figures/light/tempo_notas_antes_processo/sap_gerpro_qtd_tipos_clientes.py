...
print "nome, qtd, porcentagem, mediana, quartil_i, quartil_s, max, min"
for item, qtd in c_sorted:
    lista = sorted(nota_data[item])
    if item != "NULL": 
        array = np.asarray(lista)
        mediana = lista[len(lista) / 2] 
        quartil_i = lista[int(0.25 * len(lista))] 
        quartil_s = lista[int(0.75 * len(lista))] 
        maximo = max(lista)
        minimo = min(lista)
        print """'{}', {}, {}, {}, {}, {}, {}, {}""".format(item, qtd, qtd / total, mediana, quartil_i, quartil_s, maximo, minimo) 
    else:
        print """'{}', {}, {}""".format(item, qtd, qtd / total) 
